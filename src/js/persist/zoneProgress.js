...
export function getDefaultUnlockedZoneIds() {
    return ZONE_CONFIG.filter((zone) => zone.unlockedByDefault).map((zone) => zone.id);
}
export function readUnlockedZoneIds() {
    ...
    try {
        ...
        const ids = parsed
            .map((id) => migrateZoneId(String(id)))
...
export function writeUnlockedZoneIds(ids) {
    const defaults = getDefaultUnlockedZoneIds();
    const valid = new Set(ZONE_CONFIG.map((zone) => zone.id));
    const deduped = [...new Set([...defaults, ...ids])].filter((id) => valid.has(id));
...
export function unlockZoneIds(zoneIds) {
...
export function readDefeatedZoneBossIds() {
    try {
        ...
        const valid = new Set(ZONE_CONFIG.map((zone) => zone.id));
        return [...new Set(parsed.map((id) => migrateZoneId(String(id))).filter((id) => valid.has(id)))];
    }
    catch {
        return [];
    }
}
export function writeDefeatedZoneBossIds(ids) {
    const valid = new Set(ZONE_CONFIG.map((zone) => zone.id));
    const deduped = [...new Set(ids.map((id) => String(id)).filter((id) => valid.has(id)))];
    window.localStorage.setItem(DEFEATED_ZONE_BOSSES_KEY, JSON.stringify(deduped));
}
export function isZoneBossDefeated(zoneId) {
    if (!zoneId) {
        return false;
    }
...
export function writeDefeatedBossOpponentIds(ids) {
...
export function isBossOpponentDefeated(opponentId) {
...
export function markBossOpponentDefeated(opponentId) {
    const id = String(opponentId || "");
...
export function emptySeenZoneContent() {
    return { encounters: [], events: [], bosses: [] };
}
export function readSeenZoneContentMap() {
    try {
        ...
        const result = {};
        Object.entries(parsed).forEach(([zoneId, value]) => {
            const migratedId = migrateZoneId(zoneId);
            if (!validZones.has(migratedId) || !value || typeof value !== "object") {
                return;
            }
            const entry = value;
            result[migratedId] = {
...
export function writeBgCalendarDay(day) {
...
export function getSeenZoneContent(zoneId) {
    return readSeenZoneContentMap()[zoneId] || emptySeenZoneContent();
}
export function markSeenZoneEntry(zoneId, kind, entryId) {
    if (!zoneId || !entryId) {
        return;
    }
