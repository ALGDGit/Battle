...
import { INVENTORY_CONFIG, STAGE_ORDER, } from "./constants.js";
import { resetRunState, state } from "./state.js";
const { singlePlayerButton, arenaModeButton, exitButton, mainMenu, arenaMenu, arenaPlayerList, arenaOpponentList, arenaStartButton, arenaBackButton, characterMenu, playerList, startGameButton, backButton, actionMenu, actionBackButton, feedButton, trainButton, battleButton, finalChallengeButton, weightCounter, winRateDisplay, caretakerSprite, caretakerName, caretakerStage, caretakerHp, caretakerTypeIcon, consumableStatus, trainingCounter, feedMenu, feedBackButton, feedMeatButton, feedSpriteSlot, feedFoodSprite, feedFoodContainer, feedMushroomButton, mushroomCount, feedPepperButton, pepperCount, feedBlueAppleButton, blueAppleCount, feedMedicineButton, medicineCount, feedYellowPearButton, yellowPearCount, feedStormCloudButton, stormCloudCount, feedPoisonVialButton, poisonVialCount, feedBeerBottleButton, beerBottleCount, feedFocusLensButton, focusLensCount, feedAegisCharmButton, aegisCharmCount, feedAdrenalSeedButton, adrenalSeedCount, feedEchoShellButton, echoShellCount, feedNullPowderButton, nullPowderCount, feedWeightedAnkletButton, weightedAnkletCount, feedRationPackButton, rationPackCount, feedMutationCoreButton, mutationCoreCount, feedLuckyTokenButton, luckyTokenCount, feedLastStandEmblemButton, lastStandEmblemCount, trainMenu, trainBackButton, trainFill, trainStatus, trainRetryButton, trainSpriteSlot, trainBag, evolutionScreen, evolutionOld, evolutionNew, evolutionMessage, evolutionContinue, gameoverScreen, gameoverSprite, gameoverContinue, congratsScreen, congratsSprite, congratsContinue, battleScreen, battleBackButton, finalChallengeModal, finalChallengeConfirmButton, finalChallengeCancelButton, actionBackModal, actionBackConfirmButton, actionBackCancelButton, battlePlayerName, battleOpponentName, battlePlayerHp, battleOpponentHp, battleLog, battlePlayerSprite, battleOpponentSprite, battleField, playerStatusParalyzed, playerStatusPoisoned, playerStatusConfused, opponentStatusParalyzed, opponentStatusPoisoned, opponentStatusConfused, } = dom;
const inventoryUi = {
...
const consumableLabels = Object.fromEntries(INVENTORY_CONFIG.map((item) => [item.activeKey, item.label]));
...
async function runBattleTurn() {
    ...
    if (hit) {
        ...
        logBattle(`${attacker.name} hits with ${attack.name === "final attack"
            ? "Final Attack"
            : attack.name === "special attack"
                ? attack.displayName || "Special Attack"
                : "Standard Attack"}${critical ? " (Critical)" : ""}!`);
        ...
        logBattle(`${attacker.name} misses ${attack.name === "final attack"
            ? "Final Attack"
            : attack.name === "special attack"
                ? attack.displayName || "Special Attack"
                : "Standard Attack"}!`);
...
function applyStatus(defenderRole, status, explicitChance) {
...
function getElementProjectile(element, stageKey) {
    ...
    const safeElement = normalized === "vaccine" || normalized === "virus" || normalized === "data"
        ? normalized
        : "vaccine";
    const safeStage = stageKey || "child";
    return `data/ui/projectiles/${safeStage}_${safeElement}.svg`;
}
function getSpecialProjectile(attacker, attack) {
...
function getEvolutionCandidates(player, nextStage) {
...
function attemptEvolution(opponent) {
    ...
    if (Array.isArray(state.player.evolvesTo) && state.player.evolvesTo.length > 0) {
        ...
        pool = state.characters.filter((char) => allowed.has(String(char.id || "")) && char.stage === nextStage);
    }
    else {
