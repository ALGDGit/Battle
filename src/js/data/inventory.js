...
export function isStatDebuffId(id) {
    return STAT_DEBUFF_IDS.has(id);
}
export function getInventoryItemCategories(item) {
...
export function formatCareGainText(gains) {
    const parts = [];
...
function normalizeDropStatusId(status) {
...
function isPureStatDebuffItem(item) {
    return (item.useEffect === "opponentDebuff" &&
        Boolean(item.opponentDebuffs?.length) &&
        (item.opponentDebuffs || []).every((entry) => isStatDebuffId(entry)));
}
export function getWildDropHealKey(tier) {
...
export const INVENTORY_CONFIG = [
    ...
    },
    {
        ...
        label: "Perfect Hamburger: +1 Attack, +1 Defense, +5 Speed, +5 Intelligence permanently.",
    ...
    },
    {
        ...
        alt: "Substance X",
        label: "Substance X: If this Digimon has an X Antibody form, it evolves into that version (Agumon → Agumon X).",
        name: "Substance X",
        useEffect: "xAntibody",
    },
    {
...
export function findInventoryItemForStatus(status) {
    const id = normalizeDropStatusId(status);
    if (!id) {
        return undefined;
    }
    const buff = INVENTORY_CONFIG.find((item) => item.useEffect === "playerBuff" &&
        item.playerBuffs?.length === 1 &&
        item.playerBuffs[0] === id);
    if (buff) {
        return buff;
    }
    return INVENTORY_CONFIG.find((item) => item.useEffect === "opponentDebuff" &&
...
function listWildWinDropSlots(tier, specialStatus, specialItemKeys) {
    const slots = [
...
export function listWildWinDropKeys(tier, specialStatus, specialItemKeys) {
    return [...new Set(listWildWinDropSlots(tier, specialStatus, specialItemKeys).flat())];
}
export function pickWildWinDropKey(tier, specialStatus, inventory, specialItemKeys) {
...
const PERFECT_HAMBURGER_RECIPE = {
...
export const KEY_ITEMS_CONFIG = [
    ...
    },
    {
        ...
        label: "Incomplete Hamburger: Burgermon Mama's near-perfect burger—still missing the patty. Combine with Burger Patty in Key Items.",
    ...
    },
    {
        ...
        alt: "Burger Patty",
        label: "Burger Patty: Burgermon Papa's perfectly fried patty. Combine with Incomplete Hamburger in Key Items.",
        name: "Burger Patty",
        onUse: PERFECT_HAMBURGER_RECIPE,
    },
    {
    ...
    },
    {
        ...
        label: "First Edition Charizard Card: A legendary holographic treasure pried from GoldNumemon.",
    ...
    },
    {
        ...
        alt: "Training Manual",
        label: "Training Manual: Each successful training counts as two, but still costs 1 Weight. The 4-train cap still applies.",
...
export function countDemonLordIngredients(keyItems) {
    return DEMON_LORD_INGREDIENT_KEYS.filter((key) => (Number(keyItems?.[key]) || 0) > 0).length;
}
export function listOwnedDemonLordIngredientNames(keyItems) {
