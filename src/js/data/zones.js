...
export function isZoneUnavailable(zoneId) {
    return Boolean(zoneId && BETA_UNAVAILABLE_ZONE_IDS.includes(zoneId));
}
export function getZoneDifficultyTier(zoneId) {
...
export function getZoneDayCycleLength() {
...
export function getTimeOfDayForZoneLevel(zoneLevel) {
    const level = Math.max(1, Math.floor(Number(zoneLevel) || 1));
    const index = Math.floor((level - 1) / TIME_OF_DAY_ENCOUNTERS) % TIME_OF_DAY_ORDER.length;
    return TIME_OF_DAY_ORDER[index];
}
export function isIndoorZone(zoneId) {
    if (!zoneId) {
        return false;
    }
    return INDOOR_ZONES.includes(zoneId);
}
export function getZoneBackgroundFile(zoneId, zoneLevel = 1, calendarDay = 0) {
