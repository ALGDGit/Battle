...
export function readBannedEventIds(): string[] {
  try {
...
export function writeBannedEventIds(ids: string[]) {
...
export function isEventBannedForever(eventId: string | null | undefined): boolean {
...
export function banEventForever(eventId: string | null | undefined) {
  const id = String(eventId || "");
...
export function getExcludedEventIds(runConsumed: readonly string[] | null | undefined): string[] {
  return [...new Set([...(runConsumed || []).map(String), ...readBannedEventIds()])];
}

function readEventWinCounterMap(): Record<string, number> {
  try {
...
export function readEventWinCount(eventId: string | null | undefined): number {
  const id = String(eventId || "");
  if (!id) {
    return 0;
  }
...
export function readAllEventWinCounts(): Record<string, number> {
  return readEventWinCounterMap();
}

/** Persist a win for this event id. Returns the new total. */
export function incrementEventWinCount(eventId: string | null | undefined): number {
  const id = String(eventId || "");
  if (!id) {
    return 0;
  }
