...
export const EQUIPMENT_CONFIG = [
  ...
  },
  {
    ...
    name: "Hard Scale",
    bonuses: { defense: 2 },
    statusImmunities: ["confused", "confuse", "confuso", "confusion"] as const,
  },
  {
    key: "narwhalHorn",
  ...
  },
  {
    ...
    name: "Garurumon Coat",
    bonuses: { defense: 2 },
    statusImmunities: ["demoralized"] as const,
  },
  {
    key: "gearRing",
    ...
    name: "Gear Ring",
    bonuses: {},
    onHitEffect: { status: "swift", chance: 0.2, target: "self" as const },
  },
  {
    key: "sacredCollar",
  ...
  },
  {
    ...
    name: "Metal Coat",
    bonuses: { defense: 3 },
    statDebuffImmunities: ["defense"] as const,
  },
  {
    key: "stingerCannon",
  ...
  },
  {
    ...
    name: "Hedgehog Shell",
    bonuses: { defense: 2 },
    startStatuses: ["thorns"] as const,
  },
] as const;

export type EquipmentConfigEntry = (typeof EQUIPMENT_CONFIG)[number];
export type EquipmentKey = EquipmentConfigEntry["key"];

function titleCaseStat(stat: string): string {
  return stat.charAt(0).toUpperCase() + stat.slice(1);
}

function equipmentStatusLabel(status: string): string {
  const key = String(status || "").toLowerCase();
  if (key.startsWith("demoral")) return "Demoralized";
  if (key.startsWith("confus")) return "Confused";
  if (key.startsWith("poison")) return "Poisoned";
  if (key === "sleep" || key === "asleep" || key === "dormido") return "Asleep";
  if (key.startsWith("silen")) return "Silenced";
  if (key.startsWith("blind") || key === "cegado") return "Blinded";
  if (key.startsWith("invert")) return "Inverted";
  if (key.startsWith("burn") || key === "quemado") return "Burned";
  if (key.startsWith("vampir")) return "Vampirism";
  if (key === "thorns" || key === "espinas") return "Thorns";
  if (key.startsWith("reflect") || key === "reflejo") return "Reflector";
  if (key.startsWith("immun") || key === "inmune") return "Immune";
  if (key.startsWith("reckless") || key === "temerario" || key === "imprudente") return "Reckless";
  if (key.startsWith("curs") || key === "maldito") return "Cursed";
  if (key.startsWith("mark") || key === "marcado") return "Marked";
  if (key.startsWith("drain") || key === "drenado") return "Drained";
  if (key.startsWith("bound") || key === "atado") return "Bound";
  if (key.includes("positive") || key === "polopositivo") return "Positive Pole";
  if (key.includes("negative") || key === "polonegativo") return "Negative Pole";
  if (key.includes("deep") || key.includes("herida")) return "Deep Wound";
  if (key.startsWith("infatuat") || key === "enamorado" || key === "enamored") return "Infatuated";
  if (key === "distracted" || key === "distraido") return "Distracted";
  if (key === "swift") return "Swift";
  if (key === "slow") return "Slow";
  if (key === "exposed") return "Exposed";
  if (key === "weak") return "Weak";
  return key ? key.charAt(0).toUpperCase() + key.slice(1) : status;
}

/** Short combat effect line for hover / debug. */
export function formatEquipmentEffect(item: EquipmentConfigEntry): string {
  const parts: string[] = [];
  ...
  const gear = item as EquipmentConfigEntry & {
    statusImmunities?: readonly string[];
    onHitEffect?: { status: string; chance: number; target?: string };
    onHitEffects?: ReadonlyArray<{ status: string; chance: number; target?: string }>;
    thorns?: number;
    selfDamageOnAttack?: number;
